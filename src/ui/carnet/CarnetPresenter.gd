...
func get_profile_games(target_profile_id: String = "") -> Array:
	...
	for match in matches:
		...
		out.append({
			...
			"white_elo": int(summary.get("white_elo", 0)),
			"black_elo": int(summary.get("black_elo", 0)),
			"result": str(summary.get("result", "*")),
			"date": str(summary.get("date", "")),
			"eco": str(summary.get("eco", "")),
			"source": str(summary.get("source", "")),
			"moves_count": int(summary.get("moves_count", 0)),
			"engine_analyses_count": int(summary.get("engine_analyses_count", 0)),
