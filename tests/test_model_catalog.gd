...
func _init() -> void:
	...
	print(" - Par coup: %s" % gem_cost.label_per_query)
	assert(gem_cost.per_query_usd == 0.0, "Gemini 3.8 Flash est en Free Tier (0,00 €)")
	...
	print(" - Pour 1000 coups: %s (USD: %.4f)" % [ds_flash_cost.label_per_1000, ds_flash_cost.per_1000_usd])
	# 500 in ($0.22/1M) = 0.00011$ + 300 out ($0.66/1M) = 0.000198$ => ~0.000308$ (~0,30 $ / 1000 coups)
	...
	# 500 in ($0.66/1M) = 0.00033$ + 300 out ($1.98/1M) = 0.000594$ => ~0.000924$ (~0,92 $ / 1000 coups)
	assert(ds_pro_cost.per_query_usd > 0.0008 and ds_pro_cost.per_query_usd < 0.0011, "Coût DeepSeek V4 Pro attendu ~0.000924 $")
