...
func _init() -> void:
	...
	# 2. L'intuition couvre toujours tous les coups légaux (plus de filtre top-k).
	_check(not ComputeFinesse.profile(0).has("shallow_top_k"), "aucun filtre top-k")
	...
	_check(int(ComputeFinesse.cliff_envelope_opts(0).max_plies) == 4, "palier 0 : lignes de 4 demi-coups")
