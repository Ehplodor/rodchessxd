...
- [ ] **C5.2** `Main.gd` connecte ce signal au démarrage → appelle `_show_toast(msg, is_success)` — *présent dans le code mais non vérifié par test headless dans cette session*
...
- [x] **F1.1** Nouvelle méthode `fetch_all_player_games(username: String, max_months: int = 12, max_games: int = 500)` dans `ChessComService.gd`
...
- [x] **F1.2** Signal `progress_fetched(current: int, total: int, count: int)` pour barre progression UI
- [x] **F1.3** Gestion erreurs par mois (continue si un mois échoue, log diagnostic)

#### F2 `CarnetPresenter.bulk_import_chesscom(username, profile_name, options)` — Orchestration
- [x] **F2.1** Nouvelle méthode dans `CarnetPresenter.gd`
...
- [x] **F2.2** Options par défaut : `max_months=12`, `max_games=500`, `depth=14`, `mode="dynamic"`

#### F3 `ChessComBulkImportModal.gd` — UI (nouveau composant T3)
- [x] **F3.1** Fichier `src/ui/carnet/components/ChessComBulkImportModal.gd` (extends `Window`)
- [x] **F3.2** Champs :
...
- [x] **F3.3** Bouton "Créer carnet & importer" → appelle `presenter.bulk_import_chesscom(...)`
- [x] **F3.4** Barre progression 3 étapes :
    1. Récupération mois (`progress_fetched` signal)
    2. Import parties (compteur)
    3. Analyse (`batch_progress` signal du presenter)
- [x] **F3.5** Bouton "Annuler" → `presenter.cancel_bulk_import()` + `chess_com_service.cancel()` (nouvelle méthode)
- [x] **F3.6** Toast final : "Carnet 'X' créé avec N parties — analyse en cours..."

#### F4 Intégration dans Overlay (onglet Sync)
- [x] **F4.1** Bouton "🌐 Importer Chess.com" à côté de "📥 Importer PGN" — *stub présent*
- [x] **F4.2** Ouvre `ChessComBulkImportModal` avec `presenter` connecté
- [x] **F4.3** À la fin → `presenter.refresh_sync()` + `presenter.refresh_carnet()` + `presenter.refresh_plan()`

#### F5 Tests
- [x] **F5.1** Test unitaire : `_parse_month_games` avec JSON simulé (2/3 parties, tri inverse, user_result correct)
- [x] **F5.2** Test unitaire : `_cleanup_chesscom_service` (null, valide, invalide)
- [x] **F5.3** Test unitaire : `cancel_bulk_import` (sans service, avec service)
- [x] **F5.4** Test unitaire : modal UI (champs, valeurs par défaut, auto-nom depuis pseudo)
- [x] **F5.5** Tests existants préservés : `test_carnet_presenter.gd` 42/42, `test_carnet_overlay.gd` 24/24, `test_carnet_main_integration.gd` 6/6
- [ ] **F5.6** Test manuel : pseudo existant (ex: `hikaru`), 3 mois, 100 parties → carnet créé, parties importées, analyse lancée
- [ ] **F5.7** Test manuel : rate-limit : 60 mois → vérifie pauses 2s entre requêtes, pas d'erreur 429
- [ ] **F5.8** Test manuel : annulation : clic "Annuler" pendant fetch → stoppe requêtes, nettoie état
- [ ] **F5.9** Test manuel : dedupe : réimporter même pseudo → parties existantes non dupliquées, sync à jour
...
| `src/ui/carnet/components/ChessComBulkImportModal.gd` | **Créer** | ✅ |
| `src/network/ChessComService.gd` | Modifier : + `fetch_all_player_games` + signal `progress_fetched` | ✅ |
...
- [x] **Import Chess.com bulk** : pseudo + mois → carnet créé + parties importées + analyse lancée + progression visible
...
| `tests/test_chesscom_bulk_import.gd` | Phase F | 15 | ✅ **15/15 passent** | `_test_parse_month_games`, `_test_cleanup_chesscom_service`, `_test_cancel_bulk_import`, `_test_modal_ui` |

**Total : 87/87 tests passent.**
...
| **F (Chess.com bulk)** | **Moyenne** | **Réseau, rate-limit, pagination, annulation** | ✅ |

**Réalisé** : ~13 fichiers touchés, ~900 lignes ajoutées, 0 breaking change.
